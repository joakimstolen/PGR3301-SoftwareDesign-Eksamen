        ...
        protected override void Task()
        {
            ...
            while (!_carShop.hasCar && _running)
            {
                ...
                if (published != null)
                {
                    ...
                    Console.WriteLine("\t\t\t\t\t\t\t\t\t\t {0} purchased {1} from {2} with {3} color and {4} horsepower", _customer[index]._name, published.getType(), _salesMan[index2]._name, published.getColour(), published.getHorsePower());
                }    
            }
